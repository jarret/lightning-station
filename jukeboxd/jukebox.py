...
class Jukebox(object):
    def __init__(self, config):
        self.music_dir = config['Jukeboxd']['MusicDir']
        self.daemon_rpc = config["Jukeboxd"]['LightningRpc']
        self.queue = []
    ...
    def _init_invoices(self):
        try:
            ...
            reactor.callLater(3.0, self._init_invoices)
    ...
    def _check_paid_callback(self, result):
        ...
        if not result:
            ...
            reactor.callLater(CHECK_PERIOD, self._periodic_check)
            return
        paid, renews = result
    ...
    def run(self):
        reactor.callLater(0.5, self._periodic_check)
